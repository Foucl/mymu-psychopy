...
dev_logging.basicConfig(level=0)
...
class MyMu(exp.Experiment):
    ...
    def __init__(self,
        ...
        #self.thread.target = self.run_thread
        #self.thread.start()
        ...
        self.factors = OrderedDict({'stim_type': ['em_fac', 'nem_fac_e', 'nem_fac_f', 'scr_fac'], \
        ...
        self.blocks = OrderedDict({key: [] for key in self.factors['stim_type']})
    ...
    def run_thread(self):
        print "in thread"
        stream_info = pylsl.StreamInfo('mymu33', 'Markers', 1, 0, 'string', 'myuid')
        self.outlet = pylsl.StreamOutlet(stream_info)
        print "created outlet"
        while True:
            pass
    ...
    def set_logging(self, *args, **kwargs):
        super(MyMu, self).set_logging(level=self.log_level)
...
thismu = MyMu(rp={'no_output': False, 'debug': True, 'autorun': 0, 'happy_triangle_up': True, 
                  'block_order': [1,2,3,4], 'stream_to_LSL': True}, log_level=logging.DEBUG,
                    save_mov=False, marker_labels = None, stream_name='mymu_triggerse3')
...
#thismu = MyMu(rp={'no_output': False, 'debug': True, 'autorun': 0, 'happy_triangle_up': True, 
#                  'block_order': [1,2,3,4], 'stream_to_LSL': True}, log_level=logging.DEBUG,
#                    save_mov=False, marker_labels = None, stream_name='mymu_triggers')    							        
# -- ==mymu_class== --
